...
def run_mna_jobs(file, numRunnings, r, jr, jb, jl, jo, N_R, N_B, adjList, numNodes):
    ...
    for job in range(n_jobs):
        ...
        for mask in vector_space_generator(filtered_nodes, numNodes, cut_sol):
            ...
            if not all(available[i] for i in comb_nodes) or len(comb_nodes) > cut_sol:
               continue
    ...
    for file in files:
        ...
        for r in range(numRunnings):
            ...
            v_all_OF, v_all_nodes, v_all_sol_feasible = run_mna_jobs(
                full_name, numRunnings, r, jr, jb, jl, jo, N_R, N_B, adjList, numNodes)
            runtime = time.process_time() - start
            times_execs.append(runtime)

            save_results(r, file, full_name, edge_nodes, adjList,
                         jr, jb, jl, jo, N_R, N_B,
                         v_all_OF, v_all_nodes, v_all_sol_feasible, times_execs)
...
def save_results(r, jobs_file, full_base, edge_nodes, adjList,
    ...
    with open(output_path, 'w', encoding="utf-8") as out:
        ...
        if r == numRunnings - 1:
            ...
            df_nodes_out = pd.DataFrame({"R": V_R, "B": V_B})
