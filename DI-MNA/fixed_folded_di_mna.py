...
def find_best_comb_folding_core(
        valid_nodes, N_R, N_B, N_L, jr_job, jb_job, l_job
):
    """
    Versão paralelizada com estratégia de 'Folding' para balanceamento de carga.
    O loop externo vai apenas até N/2. Cada iteração processa o início (pesado) e o fim (leve) da lista.
    """
    n_nodes = len(valid_nodes)
    limit = n_nodes // 2    
    num_threads = numba.get_num_threads()
    
    local_best_OF = np.full(num_threads, INF, dtype=np.float64)
    local_best_comb = np.full((num_threads, 2), VALOR_INVALIDO, dtype=np.int32)

    # Loop Folded: Vai de 0 até N/2
    for k in numba.prange(limit):
        tid = numba.get_thread_id()
        
        # Dois pivôs definidos para testar: um do início (pesado) e um do fim (leve)
        idxs_to_process = np.array([k, n_nodes - 1 - k], dtype=np.int32)
        
        for i_curr in idxs_to_process:
            node_i = valid_nodes[i_curr]
            
            sin_R = N_R[node_i]
            sin_B = N_B[node_i]
            sin_L = N_L[node_i]

            # Testar nó individual
            if (sin_R >= jr_job and sin_B >= jb_job and sin_L <= l_job):
                ...
                if OF < local_best_OF[tid]:
                    local_best_OF[tid] = OF
                    local_best_comb[tid, 0] = node_i
                    local_best_comb[tid, 1] = VALOR_INVALIDO
            
            # Iteração do pivot até o fim do array de nós válidos
            for j in range(i_curr + 1, n_nodes):
                ...
                if (sum_R >= jr_job and sum_B >= jb_job and sum_L <= l_job):
                    ...
                    if OF < local_best_OF[tid]:
                        local_best_OF[tid] = OF
                        local_best_comb[tid, 0] = node_i
                        local_best_comb[tid, 1] = node_j

    # Tratamento caso N for ímpar
    if n_nodes % 2 != 0:
        mid_idx = n_nodes // 2
        node_i = valid_nodes[mid_idx]
        ...
        if (sin_R >= jr_job and sin_B >= jb_job and sin_L <= l_job):
            ...
            if OF < local_best_OF[thread_id]:
                local_best_OF[thread_id] = OF
                local_best_comb[thread_id][0] = node_i
                local_best_comb[thread_id][1] = VALOR_INVALIDO

        for j in range(mid_idx+1, n_nodes):
            ...
            # Atualiza o melhor local
                ...
                if OF < local_best_OF[thread_id]:
                    local_best_OF[thread_id] = OF
                    local_best_comb[thread_id][0] = node_i
                    local_best_comb[thread_id][1] = node_j
                    # print([node_i, node_j], OF)
        

    # Redução final
    best_thread_idx = np.argmin(local_best_OF)
    global_min_OF = local_best_OF[best_thread_idx]
    global_best_comb = local_best_comb[best_thread_idx]

    return global_best_comb, global_min_OF

# function declaration for thread using
find_best_comb_par = numba.njit(assinatura, parallel=True, fastmath=True, cache=True)(find_best_comb_folding_core)
find_best_comb = numba.njit(assinatura, parallel=False, fastmath=True, cache=True)(find_best_comb_folding_core)
